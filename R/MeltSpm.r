...
MeltSpm = function(mat.spm=NULL){
        if(class(mat.spm)!="dgCMatrix"){mat.spm=as(mat.spm, "dgCMatrix")}
        ...
        tibble::tibble(
            ...
            x=mat.spm@x
        ) %>% add_attributes(list(matrice.attr=attributes(mat.spm)[which(is_not_in(names(attributes(mat.spm)),c("i","p","Dimnames","x","factors","class")))])) %>% return
}
